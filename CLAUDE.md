...
│   │   └── common.css       # Общие CSS-переменные, slide-panel, базовые стили
│   └── js/
│       ├── layout.js        # Инъекция sidebar и header на все страницы
│       └── custom-select.js # Кастомный dropdown с поиском и inline-созданием
...
- Sidebar: единая секция (все пункты), bottom — только Выход. Компактный: 72/200px, иконки 32×32, gap 4px
- Тип фонда — кнопки `.btn-select-group`
- Модалки — slide-panel справа (400px), `.slide-panel-overlay.center` для больших форм (каскад, 680px). Не закрываются по клику вне окна. Чекбокс "Добавить ещё" над кнопкой "Сохранить"
- `js/custom-select.js` — `data-custom-select`, `data-always-search`, `data-add-new` + `data-add-handler`. Мини-панели (z-index 1100+), до 3 уровней вложенности. `initCustomSelects(root)` для динамических элементов
- Калькулятор в поле "Сумма" (operations.html) — popup с вводом выражения, вычисление через `Function()`
...
7. **Модалки** — slide-panel справа (400px, z-index 1001). `.center` для больших форм (680px). Flex-раскладка (header/body), кнопки в body. Чекбокс "Добавить ещё" над кнопкой. Не закрываются по клику вне окна. Удаление — кастомная модалка (не `confirm()`).
